  ...
  <v-app>
    <v-main>
      <div style="height: 100vh; overflow-y: scroll">
        ...
        <div>
          <div
            v-for="(panel, indexLocalStorage) in filteredPanels"
            :key="indexLocalStorage"
            class="mt-3"
          >
            <v-expansion-panels>
              <v-expansion-panel
                v-if="datos[indexLocalStorage2 + indexLocalStorage].show"
              >
                <v-expansion-panel-header
                  >Consultorio: {{ panel.clinicNumber }} / Torre:
                  {{ panel.towerNumber }} / Piso:
                  {{ panel.floorNumber }}</v-expansion-panel-header
                >
                    ...
                    <v-container>
                      <div>
                        ...
                        <v-row>
                          ...
                          >
                            <v-text-field
                              ...
                              v-model="
                                datos[indexLocalStorage2 + indexLocalStorage]
                                  .data[i].weight
                              "
                              @change="changeValue()"
                            ></v-text-field>
                        ...
                        <v-row v-for="(collection, j) in collections" :key="j">
                          <v-col
                            v-for="(dato, x) in collection.data"
                            :key="x"
          ...
          <v-pagination
            v-model="currentPage"
            :length="Math.ceil(clinics.length / itemsPerPage)"
          ></v-pagination>
        </div>
      </div>
    </v-main>
...
export default {
  ...
  computed: {
    ...
    filteredPanels() {
      let datos = this.datos.slice(
        this.paginationRange.startIndex,
        this.paginationRange.endIndex + 1
      );
  ...
  methods: {
    getCollections() {
      ...
      axios
        .post("/collector/getCollections", request)
        .then((res) => {
        ...
        .then((res) => {
          ...
          if (localStorage.getItem("collectionData")) {
            ...
            if (localData.length < this.datos2.length) {
              ...
              const objetosFaltantes = localData.filter(
                (localDato) =>
                  !this.datos2.some(
                    (dato) => dato.clinicNumber === localDato.clinicNumber
                  )
              );
    ...
    save() {
      ...
      axios
        .post("/collector/saveCollection", request)
        .then((resp) => {
          console.log(resp);
          if (resp.data.message == "Recolección registrada") {
            ...
            this.showAlert(
              "Validado",
              "Se han registrado las recolecciones con éxito",
              "success"
            );
          } else if (resp.data.message == "Datos incorrectos en la fecha") {
            this.showAlert(
              "Error",
              "Falta diligenciar el horario de recolección",
              "error"
            );
          }
        })
    ...
    update() {
      ...
      axios
        .post("/collector/updateCollection", request)
        .then((resp) => {
          console.log(resp);
          if (resp.data.message == "Modificacion registrada") {
            ...
            this.showAlert(
              "Validado",
              "Se ha registado con exitos la(s) modificacion(es)",
              "success"
            );
          } else if (
            resp.data.message == "Informacion de modificacion incompleta"
          ) {
            this.showAlert(
              "Error",
              "Falta diligenciar el horario de recolección",
    ...
    filterClinics() {
      ...
      if (
        ...
        for (let i = 0; i < this.datos2.length; i++) {
          if (
            this.datos2[i].clinicNumber.includes(
              this.clinicNumber.replace(/\./g, ",")
            )
          ) {
        ...
        for (let i = 0; i < this.datos2.length; i++) {
          if (
            this.datos2[i].towerNumber == this.towerNumber &&
            this.datos2[i].floorNumber.includes(this.floorNumber)
          ) {
            this.datos2[i].show = true;
            datosFiltrados.push(this.datos2[i]);
          } else {
        ...
        for (let i = 0; i < this.datos2.length; i++) {
          if (
            this.datos2[i].floorNumber == this.floorNumber &&
            this.datos2[i].clinicNumber.includes(this.clinicNumber)
          ) {
            this.datos2[i].show = true;
            datosFiltrados.push(this.datos2[i]);
          } else {
